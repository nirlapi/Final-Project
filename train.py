...
for epoch in range(EPOCHS):
    ...
    # === NEW GENERALIZATION PENALTY CRITERION ===
    # Penalizes the model if validation loss is high OR if the gap between train and val grows
    penalty_score = val_loss + abs(train_loss - val_loss)
    
    # Print ALL metrics to the console for monitoring
    print(f"Epoch {epoch+1:02d}/{EPOCHS} | Train Loss: {train_loss:.4f} | Train Acc: {train_acc:.4f} | Val Loss: {val_loss:.4f} | Val Acc: {val_acc:.4f} | Val F1: {val_f1:.4f} | Penalty: {penalty_score:.4f}")
    ...
    # Model Selection Strategy: Save the model only if the penalty score is strictly improving
    if penalty_score < best_penalty_score:
        best_penalty_score = penalty_score
        best_model_weights = copy.deepcopy(model.state_dict())
        print(f"  -> New best model found! (Penalty Score: {best_penalty_score:.4f})")

# Load the best weights before proceeding
if best_model_weights is not None:
    model.load_state_dict(best_model_weights)

# Save the history to CSV for the Jupyter Notebook
...
def export_test_predictions(model, loader, dataset, threshold, output_csv):
    ...
    # Map predictions back to exact file paths
...
# Run the export function using the final model state
